...
def test_tick_data_valid() -> None:
    tick = TickData(datetime=datetime.utcnow(), bid=Decimal("1.2345"), ask=Decimal("1.2346"))
...
def test_tick_data_rejects_non_positive_prices() -> None:
    with pytest.raises(ValidationError):
        TickData(datetime=datetime.utcnow(), bid=Decimal("0"), ask=Decimal("1.0"))


def test_tick_data_rejects_ask_not_above_bid() -> None:
    with pytest.raises(ValidationError):
        TickData(datetime=datetime.utcnow(), bid=Decimal("1.5"), ask=Decimal("1.5"))
...
def test_market_data_message_rejects_unknown_symbol() -> None:
    with pytest.raises(ValidationError) as excinfo:
        MarketDataMessage(
            symbol="UNKNOWN",
            timeframe="M1",
            data=TickData(datetime=datetime.utcnow(), bid=Decimal("1.0"), ask=Decimal("1.1")),
        )
